...
@Getter
public class LotteryLogicTemplate {
    private static final Random RANDOM = new Random();

    public LotteryLogicTemplate(int id) {
        this.id = id;
    }
    ...
    public void init(LotteryTemplate base) {
        if (base.getFixedSW() == 1) fixedDrop.onOff = true;
        fixedDrop.times = base.getFixedTime();
        if (Objects.nonNull(base.getFixedLoot())) {
            for (String temp : base.getFixedLoot()) {
        ...
        if (base.getWeightSW() == 1) weightDrop.onOff = true;
        weightDrop.times = base.getWeightTime();
        if (Objects.nonNull(base.getWeightLoot())) {
            int weightCounter = 0;
            for (String temp : base.getWeightLoot()) {
                if (StringUtil.isNotEmpty(temp) && !temp.equals("0")) {
                    String[] values = temp.split("_");
                    int itemId = Integer.valueOf(values[0]);
                    int min = Integer.valueOf(values[1]);
        ...
        if (base.getRandomSW() == 1) probabilisticDrop.onOff = true;
        probabilisticDrop.times = base.getRandomTime();
        probabilisticDrop.maxNum = 100;
        if (Objects.nonNull(base.getRandomLoot())) {
            for (String temp : base.getRandomLoot()) {
                if (StringUtil.isNotEmpty(temp) && !temp.equals("0")) {
                    String[] values = temp.split("_");
                    int itemId = Integer.valueOf(values[0]);
                    int min = Integer.valueOf(values[1]);
