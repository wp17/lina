...
public class Crypt {
	...
	public static byte[] hmacSHA1Encrypt(String encryptData, String encryptKey) throws Exception {
		byte[] keydata = encryptKey.getBytes("UTF-8");
		SecretKey secretKey = new SecretKeySpec(keydata, "HmacSHA256");
		Mac mac = Mac.getInstance("HmacSHA256");
		mac.init(secretKey);
		return mac.doFinal(encryptData.getBytes("UTF-8"));
	}

	public static boolean checkChallengeAnswer(long challenge, String answer) throws Exception {
		String source = Long.toString(challenge);
		byte[] dest = hmacSHA1Encrypt(source, "minaserver");
		String validate = new String(Hex.encodeHex(dest));
		if (validate.equalsIgnoreCase(answer)) {
			return true;
		}
		return false;
	}
	 ...
	 * @return
	 */
	public static int getCRC16(byte[] input, int off, int len, int seed) {
		int crc = seed;
		for (int i = 0; i < len; i++) {
			crc = (crc >> 8 ^ table[(crc ^ input[off + i]) & 0xff]);
		}
		return crc;
	}
