...
public class RankIOHandler extends SimpleChannelInboundHandler<DefaultPacket> {
    private static final Logger logger = LoggerProvider.getLogger(RankIOHandler.class);
    ...
    protected void channelRead0(ChannelHandlerContext ctx, DefaultPacket packet) {
        ...
        if (null != session) {
            RankMsgModule.getInstance().process(session, packet);
        }else {
